//
// Places Nearby controller (business logic for API endpoint)
...
export const placesController = async (request: Request, response: Response) => {
  ...
  if (!location) {
    ...
    throw new ApplicationError(400, 'No location provided');
  }
  ...
  if (Object.keys(placesNearbyResponse.data).length === 0) {
    throw new ApplicationError(404, 'No restaurants found for the given location');
  }
  ...
  placesNearbyResponse.data.places.forEach((restaurant: any) => {
    if (!restaurant.photos || !restaurant.photos[0].name) {
